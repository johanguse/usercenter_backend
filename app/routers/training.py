...
async def create_training_data(
    ...
    return await training_service.create_training_data(
        db, training_data, current_user
    )
...
async def get_training_data_by_id(
    ...
    training_data = await training_service.get_training_data_by_id(
        db, training_id, current_user
    )
...
async def delete_training_data(
    ...
    deleted = await training_service.delete_training_data(
        db, training_id, current_user
    )
    if not deleted:
        raise HTTPException(status_code=404, detail='Training data not found')
